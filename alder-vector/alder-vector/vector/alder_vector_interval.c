/**
 * This file, alder_vector_interval.c, is part of alder-vector.
 *
...
{
    alder_vector_interval_t *v = alder_vector_interval_init(1);
    for (size_t i = 0; i < n/2; i++) {
//        printf("[%zd] %d - %d\n", i, a[2*i], a[2*i+1]);
        alder_vector_interval_data_t m = { .s = a[2*i], .e = a[2*i+1] };
        alder_vector_interval_add(v, m);
    }
    alder_vector_interval_fprintf(stdout, v);
    alder_vector_interval_sort_vector_interval(v);
    printf("After sort\n");
    alder_vector_interval_fprintf(stdout, v);
    
    alder_vector_interval_free(v);
}
...
{
    alder_vector_interval_block_t * b = alder_vector_interval_block_t_alloc (n);
    if (b == 0) return 0;
    memset(b->data, 0, n * sizeof(alder_vector_interval_data_t));
    return b;
}

/* This function frees the memory used by a block b previously allocated with 
 * alder_vector_interval_block_t_alloc or
 * alder_vector_interval_block_t_calloc.
 * Argument: 
 * b - a block
 */
void
                                       ...
                                       const size_t n)
{
    ...
    alder_vector_interval_data_t *t = realloc(b->data, n * sizeof (alder_vector_interval_data_t));
    if (t == NULL) {
        GSL_ERROR_VAL ("failed to allocate space for block struct",
                       GSL_ENOMEM, 0);
    }
                                  ...
                                  const size_t n)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        const alder_vector_interval_data_t * t = data + i;
...
{
    if (!v) return;
    alder_vector_interval_block_t_free(v->block);
    free (v);
}
                           ...
                           const alder_vector_interval_data_t m)
{
    if (v->block->size == v->size) {
        // Resize a->block
        alder_vector_interval_block_t *t =
            alder_vector_interval_block_t_realloc(v->block, 2 * v->block->size);
        if (t == NULL) {
...
{
    return (alder_vector_interval_data_t *) (v->data + i);
}
...
alder_vector_interval_const_ptr (const alder_vector_interval_t * v,
                                 const size_t i)
{
    return (const alder_vector_interval_data_t *) (v->data + i);
}
...
{
    int status = alder_vector_interval_raw_fprintf (stream,
                                                   v->data,
                                                   v->size);
    return status;
}
...
static inline void my_interval_downheap (alder_vector_interval_data_t * data, const size_t stride, const size_t N, size_t k);

static inline void
my_interval_downheap (alder_vector_interval_data_t * data, const size_t stride, const size_t N, size_t k)
{
    alder_vector_interval_data_t v = data[k * stride];
...
alder_vector_interval_sort_interval (alder_vector_interval_data_t * data, const size_t stride, const size_t n)
{
    ...
    while (N > 0)
    {
        ...
        alder_vector_interval_data_t tmp = data[0 * stride];
...
{
    alder_vector_interval_sort_interval (v->data, 1, v->size) ;
}

static inline void index_interval_downheap (size_t * p, const alder_vector_interval_data_t * data, const size_t stride, const size_t N, size_t k);

static inline void
index_interval_downheap (size_t * p, const alder_vector_interval_data_t * data, const size_t stride, const size_t N, size_t k)
{
...
alder_vector_interval_sort_vector_interval_index (gsl_permutation * permutation, const alder_vector_interval_t * v)
{
    ...
    alder_vector_interval_sort_interval_index (permutation->data, v->data, 1, v->size) ;
