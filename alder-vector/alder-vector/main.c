/**
 * This file, main.c, is part of alder-vector.
 *
...
#include "alder_vector_option.h" 

int main(int argc, char * argv[])
{
    int isExitWithHelp = 0;
    alder_vector_option_t option;
    struct gengetopt_args_info args_info;
    if (my_cmdline_parser (argc, argv, &args_info) != 0) exit(1) ;
    isExitWithHelp = alder_vector_option_init(&option, &args_info);
    if (isExitWithHelp == 1) {
        my_cmdline_parser_free(&args_info);
        alder_vector_option_free(&option);
        exit(1);
    }
    
    if (args_info.interval_given > 0) {
        cmd_alder_vector_interval(args_info.interval_arg, args_info.interval_given);
    }
