/**
 * This file, alder_uf_tuple.c, is part of alder-uf.
 *
...
typedef int tree_comparison_func (const void *rb_a, const void *rb_b);
 ...
 */
static object_t *
tree_find (tree_node_t * tree, key_t query_key, tree_comparison_func *rb_compare)
{
    tree_node_t *tmp_node;
    if (tree->left == NULL) {
        return NULL;
 ...
 */
static int
tree_insert (tree_node_t * tree, key_t new_key, object_t * new_object, tree_comparison_func *rb_compare)
{
    tree_node_t *tmp_node;
    int finished;
    if (tree->left == NULL) {
...
static object_t *
tree_delete (tree_node_t * tree, key_t delete_key, tree_comparison_func *rb_compare)
{
...
int alder_uf_makeset(alder_uf_t *uf, item_t e)
{
    ...
    uf_node_t *n = tree_find (uf->tree, e, tree_compare_key);
    if (n != NULL) {
        return ALDER_ERROR_DUPLICATE;
    }
    ...
    s = tree_insert(uf->tree, e, new_uf_node, tree_compare_key);
    if (s != ALDER_SUCCESS) {
        alder_uf_assert();
    }
...
{
//    if (e1 == e2) {
    if (tree_compare_key(&e1, &e2) == 0) {
        return ALDER_ERROR_DUPLICATE;
    }
    uf_node_t *n1, *n2;
    n1 = tree_find (uf->tree, e1, tree_compare_key);
    n2 = tree_find (uf->tree, e2, tree_compare_key);
    if (n1 == NULL || n2 == NULL) {
...
int alder_uf_find(alder_uf_t *uf, item_t *e)
{
    uf_node_t *n;
    uf_node_t *root;
    n = tree_find (uf->tree, *e, tree_compare_key);
...
int alder_uf_delete(alder_uf_t *uf, item_t e)
{
    ...
    n = tree_find (uf->tree, e, tree_compare_key);
    if (n == NULL) return ALDER_ERROR_NOTEXIST;
    delete_node = tree_delete(uf->tree, e, tree_compare_key);
...
void alder_uf_print(alder_uf_t *uf, FILE *fp, int i)
{
    ...
    while (!uf_stack_empty(stack)) {
        ...
        fprintf(fp, "a%lldb%lld [label=\"%lld-%lld (%d:%d)",
                ...
                t->rank, t->degree);
        if (t->left != NULL) {
            fprintf(fp, " %lld-%lld", t->left->item->item.a, t->left->item->item.b);
        } else {
            fprintf(fp, " x");
        }
        if (t->right != NULL) {
            fprintf(fp, " %lld-%lld", t->right->item->item.a, t->right->item->item.b);
        } else {
            fprintf(fp, " x");
        }
        if (t->down != NULL) {
            fprintf(fp, " %lld-%lld", t->down->item->item.a, t->down->item->item.b);
        } else {
            fprintf(fp, " x");
        }
        fprintf(fp, "\"];\n");
