...
@implementation XFKeyBuilder

/* ================================================= Class Methods ================================================== */
+ (XFKeyBuilder*)forItemNamed:(NSString*)name
{
    NSData* data = [name dataUsingEncoding:NSUTF8StringEncoding];
    return [[XFKeyBuilder alloc] initHashValueMD5HashWithBytes:[data bytes] length:[data length]];

}

+ (XFKeyBuilder*)createUnique
{
    CFUUIDRef theUUID = CFUUIDCreate(NULL);
    CFUUIDBytes bytes = CFUUIDGetUUIDBytes(theUUID);
    CFRelease(theUUID);

    return [[XFKeyBuilder alloc] initHashValueMD5HashWithBytes:&bytes length:sizeof(bytes)];
}
