...
@interface XFSubProjectDefinition ()
@property(nonatomic, strong, readwrite) NSString* relativePath;
@end

@implementation XFSubProjectDefinition
...
+ (XFSubProjectDefinition*)withName:(NSString*)name path:(NSString*)path parentProject:(XFProject*)parentProject
{
    ...
    return [[XFSubProjectDefinition alloc] initWithName:name path:path parentProject:parentProject];
}
...
- (id)initWithName:(NSString*)name
              path:(NSString*)path
     parentProject:(XFProject*)parentProject
{
    self = [super init];
    if (self)
    {
        ...
        _subProject = [[XFProject alloc]
                       initWithFilePath:[NSString stringWithFormat:@"%@/%@.xfastproj", path, name]
                       baseDirectory:path];
    }
    return self;
}
...
{
    return [_name stringByAppendingString:@".xfastproj"];
}

- (NSString*)fullPathName
{
    return [NSString stringWithFormat:@"%@/%@", _path, [_name stringByAppendingString:@".xfastproj"]];
}
...
- (NSArray*)buildProductNames
{
    ...
    {
        if ([[obj valueForKey:@"isa"] asMemberType] == XFPBXProjectType)
        {
...
// returns the key of the XFPBXFileReference of the xcodeproj file
- (NSString*)projectKey
{
    if (_key == nil)
    {
        NSArray* xcodeprojKeys =
            [_parentProject keysForProjectObjectsOfType:XFPBXFileReferenceType withIdentifier:[self pathRelativeToProjectRoot] singleton:YES
                required:YES];
...
{
    return [NSString stringWithFormat:@"XfastprojDefinition: sourceFileName = %@, path=%@, type=%d", _name, _path, _type];
}
