/**
 * This file, main.c, is part of alder-cigar.
 *
...
#include <stdlib.h>
#include "cmdline.h"
#include "alder_cigar_option.h" 

int main(int argc, char * argv[])
{
    int isExitWithHelp = 0;
    alder_cigar_option_t option;
    struct gengetopt_args_info args_info;
    if (my_cmdline_parser (argc, argv, &args_info) != 0) exit(1) ;
    isExitWithHelp = alder_cigar_option_init(&option, &args_info);
    if (isExitWithHelp == 1) {
        my_cmdline_parser_free(&args_info);
        alder_cigar_option_free(&option);
        exit(1);
    }
