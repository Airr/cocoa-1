...
{
    brass_stack_t *st;
    st = (brass_stack_t *) malloc (sizeof (brass_stack_t));
...
int
stack_empty (brass_stack_t * st)
{
    return (st->base == st->top && st->previous == NULL);
}

void
push (item_t x, brass_stack_t * st)
{
    ...
    {
        brass_stack_t *new;
        new = (brass_stack_t *) malloc (sizeof (brass_stack_t));
        new->base = st->base;
        new->top = st->top;
        new->size = st->size;
