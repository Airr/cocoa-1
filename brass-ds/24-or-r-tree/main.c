          ...
          while (stack_p > 0)
            {
              ...
              if (t_tmp->l_dim_tree != NULL)
                t_tmp->l_dim_tree = build_or_r_tree ((p_list_node_t *) t_tmp->
                                                     l_dim_tree, dim - 1);
              if (t_tmp->right != NULL)
                {
                  node_stack[stack_p++] = t_tmp->left;
      ...
      while (current_node->right != NULL)       /* not at leaf */
        {
          ...
          else if (a[dim] < current_node->key && current_node->key < b[dim])    /* split: a < key < b */
            {
      ...
      if (left_path != NULL)
        {                       /* now follow the path of the left endpoint a */
          while (left_path->right != NULL)
            {
              if (a[dim] < left_path->key)
                {               /* right node must be selected */
                  new_list =
                    find_points (left_path->right->l_dim_tree, a, b, dim - 1);
          ...
          if (left_path->right == NULL && left_path->key == a[dim])
            {
              new_list = find_points (left_path->l_dim_tree, a, b, dim - 1);
              current_list = join_list (new_list, current_list);
            }
        }                       /* end left path */
      if (right_path != NULL)
        {                       /* and now follow the path of the right endpoint b */
          while (right_path->right != NULL)
            {
              if (right_path->key < b[dim])
                {               /* left node must be selected */
                  new_list =
                    find_points (right_path->left->l_dim_tree, a, b, dim - 1);
                  current_list = join_list (new_list, current_list);
          ...
          if (right_path->right == NULL && right_path->key < b[dim])
            {
              new_list = find_points (right_path->l_dim_tree, a, b, dim - 1);
              current_list = join_list (new_list, current_list);
            }
        }                       /* end right path */
    }
...
main ()
{
  ...
  for (pt_tmp = pt_list; pt_tmp != NULL; pt_tmp = pt_tmp->next)
    printf ("(%d,%d) ", pt_tmp->point->coordinate[0],
            pt_tmp->point->coordinate[1]);
      ...
      if (nextop == 'f')
        {
          ...
          while (answer_list != NULL)
            {
              printf (" (%d,%d) ", answer_list->point->coordinate[0],
                      answer_list->point->coordinate[1]);
