/**
 * This file, alder_kmer_thread4.c, is part of alder-kmer.
 *
...
#define ALDER_KMER_COUNTER4_INBUFFER_I_NP        1
#define ALDER_KMER_COUNTER4_INBUFFER_LENGTH      9
#define ALDER_KMER_COUNTER4_INBUFFER_BODY        17
 ...
 */
static bstring
filename_partition(alder_kmer_thread4_t *o, uint64_t i_np)
{
    /* File name setup */
    bstring bfpar = NULL;
    if (o->infile == NULL) {
 ...
 */
static int
compute_number_block(alder_kmer_thread4_t *o, uint64_t i_np)
{
    ...
    //    size_t size_body = o->size_subinbuf - ALDER_KMER_COUNTER4_INBUFFER_BODY;
...
static void
alder_kmer_thread4_destroy(alder_kmer_thread4_t *o)
{
    if (o != NULL) {
...
static alder_kmer_thread4_t *
alder_kmer_thread4_create(FILE *fpout,
                          int n_counter,
                          uint64_t i_ni,
                          int kmer_size,
                          ...
                          const char *outfile)
{
    alder_log5("Creating alder_kmer_thread4_t...");
    alder_kmer_thread4_t *o = malloc(sizeof(*o));
    if (o == NULL) {
        alder_loge(ALDER_ERR_MEMORY, "cannot create alder_kmer_thread4_t");
        return NULL;
    }
    memset(o, 0, sizeof(*o));
    
    /* init */
    o->size_writebuffer = 1 << 20; // FIXME: must be determined.
    o->k = kmer_size;
    ...
    o->lock_reader = ALDER_KMER_COUNTER4_READER_UNLOCK;
    o->lock_writer = ALDER_KMER_COUNTER4_WRITER_UNLOCK;
    o->next_lenbuf = 0;
    o->reader_lenbuf = 0;
    o->reader_i_parfile = 0;
    //    o->size_inbuf = (size_t)(memory_available << 19);
    o->size_subinbuf = ALDER_KMER_COUNTER4_INBUFFER_BODY + BUFSIZE + o->b;
    ...
    /* adjust size inbuf */
    //    if (o->size_subinbuf < ALDER_KMER_COUNTER4_INBUFFER_BODY) {
    ...
    //        return NULL;
    //    }
    //    o->size_subinbuf -= ALDER_KMER_COUNTER4_INBUFFER_BODY;
...
static int close_parfile (alder_kmer_thread4_t *o)
{
    assert(o != NULL);
    XFCLOSE(o->fpin);
    return ALDER_STATUS_SUCCESS;
 ...
 */
static int
open_parfile(alder_kmer_thread4_t *o, uint64_t i_np)
{
    bstring bfpar = filename_partition(o, i_np);
    if (bfpar == NULL) {
        return ALDER_STATUS_ERROR;
...
//    fcntl(fileno(o->fpin), F_NOCACHE, 1);
    fcntl(fileno(o->fpin), F_RDAHEAD, 1); // FIXME: profile it.
    if (o->fpin == NULL) {
                                ...
                                const char *outfile)
{
    ...
    alder_kmer_thread4_t *data = alder_kmer_thread4_create(fpout,
                                                           n_counter,
                                                           i_ni,
                                                           kmer_size,
                                                           memory_available,
    ...
    if (data == NULL || threads == NULL) {
        ...
        XFREE(threads);
        alder_kmer_thread4_destroy(data);
        return ALDER_STATUS_ERROR;
    }
...
write_tabfile(alder_kmer_thread4_t *a, size_t c_table, uint64_t i_np)
{
    // c_table of main i_np must enter first.
    int s;
    //    assert(a->ht[c_table]->i_np == a->main_i_np);
    
    alder_kmer_counter4_lock_writer(a, i_np);
...
//                                                    a->n_np);
    ...
    a->main_i_np++;
    alder_kmer_counter4_unlock_writer(a);
    return a->n_ht;
}
...
read_parfile(alder_kmer_thread4_t *a, int counter_id)
{
    int s;
    ...
    uint8_t *inbuf_body = inbuf + ALDER_KMER_COUNTER4_INBUFFER_BODY;
...
//    lenbuf = fread(inbuf_body, sizeof(*inbuf_body), BUFSIZE, a->fpin);
    lenbuf = read(fileno(a->fpin), inbuf_body, BUFSIZE);
...
//            lenbuf = fread(inbuf_body, sizeof(*inbuf_body), BUFSIZE, a->fpin);
    ...
    to_uint64(inbuf,ALDER_KMER_COUNTER4_INBUFFER_I_NP) = a->reader_i_parfile - 1;
    to_size(inbuf,ALDER_KMER_COUNTER4_INBUFFER_LENGTH) = a->reader_lenbuf;
...
static void *counter(void *t)
{
    ...
    alder_kmer_thread4_t *a = (alder_kmer_thread4_t*)t;
    assert(a != NULL);
    ...
    while (1) {
        ...
        /* setup of inbuf */
        uint8_t *inbuf = a->inbuf + c_inbuffer * a->size_subinbuf;
        i_np = to_uint64(inbuf,ALDER_KMER_COUNTER4_INBUFFER_I_NP);
        size_t lenbuf = to_size(inbuf,ALDER_KMER_COUNTER4_INBUFFER_LENGTH);
        uint8_t *inbuf_body = inbuf + ALDER_KMER_COUNTER4_INBUFFER_BODY;
        /* setup of table */
        ...
        if (lenbuf > 0) {
            alder_kmer_counter4_increment_n_block(a, i_np);
        }
