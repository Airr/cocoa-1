...
{
    /* lock */
    int oval = ALDER_KMER_COUNTER3_READER_UNLOCK;
    int cval = ALDER_KMER_COUNTER3_READER_LOCK;
    while (cval != oval) {
        oval = a->lock_reader;
...
#endif
        cval = oval + 1;
        if (oval == ALDER_KMER_COUNTER3_READER_UNLOCK) {
...
{
    /* lock */
    int oval = ALDER_KMER_COUNTER3_READER_LOCK;
    int cval = ALDER_KMER_COUNTER3_READER_UNLOCK;
    while (cval != oval) {
        oval = a->lock_reader;
        cval = oval - 1;
        if (oval == ALDER_KMER_COUNTER3_READER_LOCK) {
            cval = __sync_val_compare_and_swap(&a->lock_reader,
                                               (int)oval, (int)cval);
        }
    }
...
{
    /* lock */
    int oval = ALDER_KMER_COUNTER3_WRITER_UNLOCK;
    int cval = ALDER_KMER_COUNTER3_WRITER_LOCK;
    while (cval != oval) {
        oval = a->lock_writer;
#if defined(ALDER_THREAD)
...
#endif
        cval = oval + 1;
        if (oval == ALDER_KMER_COUNTER3_WRITER_UNLOCK &&
            a->main_i_np == i_np) {
...
{
    /* lock */
    int oval = ALDER_KMER_COUNTER3_WRITER_LOCK;
    int cval = ALDER_KMER_COUNTER3_WRITER_UNLOCK;
    while (cval != oval) {
        oval = a->lock_writer;
        cval = oval - 1;
        if (oval == ALDER_KMER_COUNTER3_WRITER_LOCK) {
            cval = __sync_val_compare_and_swap(&a->lock_writer,
                                               (int)oval, (int)cval);
        }
    }
