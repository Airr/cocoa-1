/**
 * This file, alder_kmer_encode4.c, is part of alder-kmer.
 *
...
#define ALDER_KMER_ENCODER4_OUTBUFFER_HEADER     1
#define ALDER_KMER_ENCODER4_OUTSUBBUFFER_HEADER  8

//
#define ALDER_KMER_ENCODER4_OUTBUFFER_BODY       8

#define ALDER_KMER_ENCODER4_OUTBUFFER_A          0
#define ALDER_KMER_ENCODER4_OUTBUFFER_B          8
#define ALDER_KMER_ENCODER4_OUTBUFFER_C          16

#define ALDER_KMER_ENCODER4_INBUFFER_TYPE_INFILE 1
#define ALDER_KMER_ENCODER4_INBUFFER_CURRENT     2
#define ALDER_KMER_ENCODER4_INBUFFER_LENGTH      10
#define ALDER_KMER_ENCODER4_INBUFFER_BODY        18
#define ALDER_KMER_ENCODER4_FILETYPE_FASTA       1
#define ALDER_KMER_ENCODER4_FILETYPE_FASTQ       2
#define ALDER_KMER_ENCODER4_FILETYPE_SEQ         3
...
static void
alder_kmer_encoder4_destroy (alder_kmer_encoder4_t *o)
{
    if (o != NULL) {
        if (o->fpout != NULL) {
...
static alder_kmer_encoder4_t *
alder_kmer_encoder4_create(int n_encoder,
                           int i_iteration,
                           int kmer_size,
                           long disk_space,
                           ...
                           const char *outfile)
{
    ...
    alder_kmer_encoder4_t *o = malloc(sizeof(*o));
    if (o == NULL) {
...
//    o->size_fixedoutbuffer = 7; /* FIXME: this must be determined before the function call */
    ...
    o->lock_reader = ALDER_KMER_ENCODER4_READER_UNLOCK;
    o->lock_writer = ALDER_KMER_ENCODER4_WRITER_UNLOCK;
    o->reader_lenbuf = 0;
    o->reader_lenbuf2 = 0;
    ...
    o->size_subinbuf = ALDER_KMER_ENCODER4_INBUFFER_BODY + BUFSIZE + o->size_inbuf2;
...
//        int fd = fileno(o->fpout[i]);
    ...
    //        *outbuf = ALDER_KMER_ENCODER4_EMPTY_OUTBUF;
    //    }
...
void
alder_kmer_encoder4_destroy_with_error (alder_kmer_encoder4_t *o, int s)
{
    if (s != 0) {
 ...
 */
static int close_infile (alder_kmer_encoder4_t *o)
{
    assert(o != NULL);
 ...
 *  @return SUCCESS or FAIL
 */
static int open_infile (alder_kmer_encoder4_t *o, int i_infile)
{
    int fp = -1;
    gzFile fpgz = NULL;
    BZFILE *fpbz = NULL;
 ...
 */
static int
alder_kmer_encode4_token(size_t *curbuf, uint8_t *inbuf, size_t lenbuf,
                         int type_infile)
{
    char c;
    ...
    if (type_infile == ALDER_KMER_ENCODER4_FILETYPE_FASTA) {
        if (c == '>' || c == '\n') {
            /*****************************************************************/
            /*                          OPTIMIZATION                         */
            /*****************************************************************/
    ...
    } else if (type_infile == ALDER_KMER_ENCODER4_FILETYPE_FASTQ) {
        if (c == '@') {
            // header
            /*****************************************************************/
            /*                          OPTIMIZATION                         */
        ...
        assert(type_infile == ALDER_KMER_ENCODER4_FILETYPE_SEQ);
        if (c == '\n') {
            if (vcurbuf < lenbuf) {
                // no code.
            } else {
 ...
 */
int
alder_kmer_encode4(int n_encoder,
                   int i_iteration,
                   int kmer_size,
                   long disk_space,
                   long memory_available,
                   ...
                   const char *outfile)
{
    ...
    alder_kmer_encoder4_t *data =
    alder_kmer_encoder4_create(n_encoder,
                               i_iteration,
                               kmer_size,
                               disk_space,
                               memory_available,
    ...
    if (data == NULL || threads == NULL) {
        ...
        XFREE(threads);
        alder_kmer_encoder4_destroy(data);
        return ALDER_STATUS_ERROR;
    }
 ...
 */
static uint8_t what_type_infile(alder_kmer_encoder4_t *a)
{
    uint8_t type_infile;
    if (a->type_infile & ALDER_FILETYPE_ISFASTA) {
        type_infile = ALDER_KMER_ENCODER4_FILETYPE_FASTA;
    } else if (a->type_infile & ALDER_FILETYPE_ISFASTQ) {
        type_infile = ALDER_KMER_ENCODER4_FILETYPE_FASTQ;
    } else {
        type_infile = ALDER_KMER_ENCODER4_FILETYPE_SEQ;
    }
    return type_infile;
}
 ...
 */
//#define VERSION4_DEV
static size_t write_to_partition_file(alder_kmer_encoder4_t *a,
                                   int encoder_id,
                                   int partition_id)
{
...
#endif
    alder_kmer_encoder4_lock_writer(a, partition_id);
    ...
    uint8_t *B_outbuf2 = outbuf2 + ALDER_KMER_ENCODER4_OUTBUFFER_B;
    uint8_t *C_outbuf2 = outbuf2 + ALDER_KMER_ENCODER4_OUTBUFFER_C;
    uint8_t *D_outbuf2 = outbuf2 + ALDER_KMER_ENCODER4_OUTBUFFER_C + a->b;
    uint8_t *E_outbuf2 = outbuf2 + ALDER_KMER_ENCODER4_OUTBUFFER_C + a->b + a->size_fixedoutbuffer;
    uint8_t *F_outbuf2 = outbuf2 + ALDER_KMER_ENCODER4_OUTBUFFER_C + a->b + a->size_fixedoutbuffer + a->b;
    ...
    uint8_t *OB_outbuf2 = O_outbuf2 + ALDER_KMER_ENCODER4_OUTBUFFER_B;
    uint8_t *OF_outbuf2 = O_outbuf2 + ALDER_KMER_ENCODER4_OUTBUFFER_C + a->b + a->size_fixedoutbuffer + a->b;
    ...
    alder_kmer_encoder4_unlock_writer(a, partition_id);
...
static int read_from_sequence_file(alder_kmer_encoder4_t *a, int encoder_id)
{
    int s;
    alder_kmer_encoder4_lock_reader(a, encoder_id);
    ...
    if (a->reader_lenbuf == 0) {
        ...
        if (a->reader_i_infile < a->infile->qty) {
            ...
            // in encoder function.
            alder_kmer_encoder4_unlock_reader(a, encoder_id);
    ...
    inbuf[ALDER_KMER_ENCODER4_INBUFFER_TYPE_INFILE] = a->reader_type_infile;
    uint8_t *inbuf_body = inbuf + ALDER_KMER_ENCODER4_INBUFFER_BODY;
    size_t size_body = a->size_subinbuf - ALDER_KMER_ENCODER4_INBUFFER_BODY;
    ...
    to_size(inbuf, ALDER_KMER_ENCODER4_INBUFFER_CURRENT) = (size_t)0;
    to_size(inbuf, ALDER_KMER_ENCODER4_INBUFFER_LENGTH) = a->reader_lenbuf;
...
static void *encoder(void *t)
{
    ...
    //    int s = ALDER_STATUS_SUCCESS;
    alder_kmer_encoder4_t *a = (alder_kmer_encoder4_t*)t;
    assert(a != NULL);
    ...
    mpz_t mpzS1, mpzS2, mpzSS;
    mpz_init2(mpzS1, s1->b * 8);
    mpz_init2(mpzS2, s1->b * 8);
    mpz_init2(mpzSS, s1->b * 8);
//    int n_boundary = (a->k / ALDER_NUMKMER_8BYTE);
    int n_bits = (a->k / ALDER_NUMKMER_8BYTE + a->k) * 2;
    mpz_t *mpzDNA = malloc(sizeof(*mpzDNA)*4);
    for (unsigned long int i = 0; i < 4; i++) {
        mpz_init2(mpzDNA[i], s1->b * 8);
        mpz_set_ui(mpzDNA[i],i);
        mpz_mul_2exp(mpzDNA[i],mpzDNA[i],n_bits-2);
    }
    ...
    while (1) {
        ...
        /* setup of inbuf */
        uint8_t *inbuf = a->inbuf + c_inbuffer * a->size_subinbuf;
        //        assert(*inbuf == ALDER_KMER_ENCODER4_ENCODER_USE_INBUF);
        int type_infile = (int)inbuf[ALDER_KMER_ENCODER4_INBUFFER_TYPE_INFILE];
        assert(type_infile == ALDER_KMER_ENCODER4_FILETYPE_FASTA ||
               type_infile == ALDER_KMER_ENCODER4_FILETYPE_FASTQ ||
               type_infile == ALDER_KMER_ENCODER4_FILETYPE_SEQ);
        
        size_t curbuf = to_size(inbuf,ALDER_KMER_ENCODER4_INBUFFER_CURRENT);
        size_t lenbuf = to_size(inbuf,ALDER_KMER_ENCODER4_INBUFFER_LENGTH);
        uint8_t *inbuf_body = inbuf + ALDER_KMER_ENCODER4_INBUFFER_BODY;
        /* setup of outbuf */
        ...
        while (token < 5 && full_partition == a->n_np) {
            ...
            if (token == 4) {
                ...
                for (int i = 0; i < a->k - 1; i++) {
                    ...
                    if ((a->k - 1 - i) % ALDER_NUMKMER_8BYTE) {
                        mpz_mul_2exp(mpzS1,mpzS1,2);
                    } else {
                        mpz_mul_2exp(mpzS1,mpzS1,4);
                    }
                    mpz_add_ui(mpzS1,mpzS1,b1);
                    
                    if ((i + 1) % ALDER_NUMKMER_8BYTE) {
                        mpz_tdiv_q_2exp(mpzS2,mpzS2,2);
                    } else {
                        mpz_tdiv_q_2exp(mpzS2,mpzS2,4);
                    }
                    mpz_add(mpzS2,mpzS2,mpzDNA[b2]);
#endif
                }
                if (token >= 4) {
                    continue;
            ...
            mpz_mul_2exp(mpzS1,mpzS1,2);
            mpz_add_ui(mpzS1,mpzS1,b1);
            for (int i_boundary = 0; i_boundary < n_boundary; i_boundary++) {
                if (mpz_tstbit(mpzS1,62 + 64 * i_boundary)) {
                    mpz_clrbit(mpzS1,62 + 64 * i_boundary);
                    mpz_setbit(mpzS1,64 + 64 * i_boundary);
                } else {
                    mpz_clrbit(mpzS1,64 + 64 * i_boundary);
                }
                if (mpz_tstbit(mpzS1,62+1 + 64 * i_boundary)) {
                    mpz_clrbit(mpzS1,62+1 + 64 * i_boundary);
                    mpz_setbit(mpzS1,64+1 + 64 * i_boundary);
                } else {
                    mpz_clrbit(mpzS1,64+1 + 64 * i_boundary);
                }
            }
            mpz_clrbit(mpzS1,n_bits);
            mpz_clrbit(mpzS1,n_bits+1);
            
            mpz_tdiv_q_2exp(mpzS2,mpzS2,2);
            mpz_add(mpzS2,mpzS2,mpzDNA[b2]);
            for (int i_boundary = 0; i_boundary < n_boundary; i_boundary++) {
                if (mpz_tstbit(mpzS2,62 + 64 * i_boundary)) {
                    mpz_clrbit(mpzS2,62 + 64 * i_boundary);
                    mpz_setbit(mpzS2,60 + 64 * i_boundary);
                } else {
                    mpz_clrbit(mpzS2,60 + 64 * i_boundary);
                }
                if (mpz_tstbit(mpzS2,62+1 + 64 * i_boundary)) {
                    mpz_clrbit(mpzS2,62+1 + 64 * i_boundary);
                    mpz_setbit(mpzS2,60+1 + 64 * i_boundary);
                } else {
                    mpz_clrbit(mpzS2,60+1 + 64 * i_boundary);
                }
            }
            size_t countp = 0;
            mpz_export(s1->n, &countp, -1, 8, 0, 0, mpzS1);
            mpz_export(s2->n, &countp, -1, 8, 0, 0, mpzS2);
            
            for (int i = 0; i < m1->s; i++) {
                assert(s1->n[i].key64 == m1->n[i].key64);
            }
#endif
            ...
            if (i_ni == a->i_ni) {
                uint8_t *D_outbuf2 = (outbuf + i_np * a->size_suboutbuf2 +
                                      ALDER_KMER_ENCODER4_OUTBUFFER_C + a->b);
...
//        }
        ...
        if (token == 5) {
            ...
            to_size(inbuf,ALDER_KMER_ENCODER4_INBUFFER_CURRENT) = curbuf;
