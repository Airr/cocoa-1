...
const char *my_cmdline_parser_select_version_values[] = {"0", "1", "2", "3", "4", 0}; /*< Possible values for select-version. */
...
void clear_args (struct gengetopt_args_info *args_info)
{
  ...
  args_info->select_version_arg = 4;
  ...
  while (1)
    {
      ...
      switch (c)
        ...
        case 0:	/* Long option with no short option */
          ...
          /* Select a version of kmer counting: 0 for automatic selction, 1 for skipping binary file creation (dsk mode with a single iteration), 2 for binary file creation (dsk mode), 3 for loading a whole input data on the memory (msk or memory streaming kmer mode), and 4 for splitting a binary file..  */
          else if (strcmp (long_options[option_index].name, "select-version") == 0)
          {
            ...
            if (update_arg( (void *)&(args_info->select_version_arg), 
                ...
                &(local_args_info.select_version_given), optarg, my_cmdline_parser_select_version_values, "4", ARG_LONG,
                check_ambiguity, override, 0, 0,
                "select-version", '-',
                additional_error))
              goto failure;
