...
const char *gengetopt_args_info_description = "alder-kmer allows arbitrary size of kmer using multiple CPUs with\ncompare-and-swap (CAS) instructions. The first argument of the alder-kmer is a\ncommand, which is one of the strings: `count', `dump', `query', `simulate',\n`partition', `kmer', `table', `list', `match', `binary', `uncompress', and\n`inspect'. Use option --help to see options for each command; e.g., alder-kmer\ncount --help. Options may require arguments. `INT' is a positive number unless\nstated otherwise. `STRING' is a string of characters. `FILENAME' and\n`DIRECTORY' are strings that could be found or created in the file system that\nyou run alder-kmer on. These are common options, which are followed by commands\nand their main options.";
...
const char *gengetopt_args_info_detailed_help[] = {
  ...
  "\ncount:",
  "  The input files must be sequence files: fasta, or fastq. With no input files,\n  but with STDIN from other program, the first stage of generating a binary\n  file would do the computation of total number of kmers. With input files, the\n  table file would be written to STDOUT unless --outfile is specified. A table\n  file is accompanied by its table header .tbh file. The numbers of iterations\n  and partitions are determined by the available disk space, memory, and input\n  sequence data (see Rizk et al. 2013 using --cite option). The program\n  attempts to compute the available disk space and memory at the time the\n  program runs. This help message shows the available disk space and memory\n  below. Those values can be changed whenever the program runs, and they can be\n  different from the ones below. You could specify disk space or available\n  memory, and adjust the usage of disk space and memory using load options:\n  --load-disk and --load-memory. You can also specify the number of iterations\n  and partitions --ni and --np, these will override values that otherwise would\n  be determined by disk space and input sequence data; this is not recommended\n  for real data, but for command `simulate'. Output file name prefix can be\n  changed using --outfile, and so can their location with --outdir. In version\n  6, the given total memory is filled with input data, and the memory left-over\n  is used for a partition table. If the minimum memory for a partition table is\n  not met, the version 6 is reduced to version 5. Because too large table may\n  failed to be loaded onto the memory space in other computers, the maximum\n  memory for a partition table can be set as well.",
  ...
  "\nquery:",
  "  NOT WORKING or BUGs - Query a count table in a kmer count table; this is\n  given by option --tabfile or the default name can be changed using option\n  --outfile. A string of DNA sequence can be given with option --sequence, or a\n  set of DNA sequences can be given by FASTA or FASTQ files.",
  "      --query                  dummy  (default=off)",
  "      --sequence=sequence      Query a kmer DNA sequence",
  "\nsimulate:",
  "  This command creates FASTQ files for testing alder-kmer. Use commands `list'\n  or `binary' to work with generated FASTQ files. There are no input files for\n  `simulate' command. By default, a sequence file would be written to STDOUT if\n  --nf option is given with 1 or a single sequence file is requested. Override\n  this feature by using option --outfile. First, use --kmer to specifiy the\n  length of kmer sequences. Set the number of kmers using --maxkmer. Use --nf\n  to change the number of files. Turn on --with-parfile option to create\n  partition files as well. Total number of parition files with .par extension\n  is equal to ni x np where ni is given by option --ni and np.  See the\n  reference papers using option --cite.",
...
init_full_help_array(void)
{
  ...
  gengetopt_args_info_full_help[104] = 0; 
  
}

const char *gengetopt_args_info_full_help[105];
...
init_help_array(void)
{
  ...
  gengetopt_args_info_help[103] = 0; 
  
}

const char *gengetopt_args_info_help[104];
...
const char *my_cmdline_parser_select_version_values[] = {"0", "1", "2", "3", "4", "5", "7", 0}; /*< Possible values for select-version. */
...
void init_args_info(struct gengetopt_args_info *args_info)
{
  ...
  args_info->release_help = gengetopt_args_info_detailed_help[95] ;
  args_info->exit_help = gengetopt_args_info_detailed_help[98] ;
  args_info->examples_help = gengetopt_args_info_detailed_help[101] ;
  args_info->document_help = gengetopt_args_info_detailed_help[104] ;
  ...
  while (1)
    {
      ...
      switch (c)
        ...
        case 0:	/* Long option with no short option */
          ...
          /* Select a version of kmer counting: 0 for automatic selction, 1 for skipping binary file creation (dsk mode with a single iteration), 2 for binary file creation (dsk mode), 3 for loading a whole input data on the memory (msk or memory streaming kmer mode), 4 for splitting a binary file, 5 for distributed reading, and 7 for sequential read with CAS encoding..  */
          else if (strcmp (long_options[option_index].name, "select-version") == 0)
          {
            ...
            if (update_arg( (void *)&(args_info->select_version_arg), 
                ...
                &(local_args_info.select_version_given), optarg, my_cmdline_parser_select_version_values, "7", ARG_LONG,
                check_ambiguity, override, 0, 0,
                "select-version", '-',
                additional_error))
              goto failure;
