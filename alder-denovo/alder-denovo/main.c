/**
 * This file, main.c, is part of alder-denovo.
 *
...
#include <stdlib.h>
#include "cmdline.h"
#include "alder_denovo_option.h" 
#include "alder_denovo.h"

int main(int argc, char * argv[])
{
    int isExitWithHelp = 0;
    alder_denovo_option_t option;
    struct gengetopt_args_info args_info;
    if (my_cmdline_parser (argc, argv, &args_info) != 0) exit(1) ;
    isExitWithHelp = alder_denovo_option_init(&option, &args_info);
    if (isExitWithHelp == 1) {
        my_cmdline_parser_free(&args_info);
        alder_denovo_option_free(&option);
        exit(1);
    }
    
    
    if (args_info.match_flag) {
        alder_denovo_match(option.infile);
    }
    if (args_info.cluster_flag) {
        alder_denovo_cluster(option.infile);
    }
