/**
 * This file, main.c, is part of alder-bitarray.
 *
...
int main(int argc, char * argv[])
{
    ...
    alder_bitarray_option_t option;
    struct gengetopt_args_info args_info;
    if (my_cmdline_parser (argc, argv, &args_info) != 0) exit(1) ;
    isExitWithHelp = alder_bitarray_option_init(&option, &args_info);
    if (isExitWithHelp == 1) {
        my_cmdline_parser_free(&args_info);
        alder_bitarray_option_free(&option);
        exit(1);
    }
