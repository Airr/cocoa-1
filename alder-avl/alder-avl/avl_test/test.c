...
stoi (const char *s)
{
  ...
  return (int)(x >= INT_MIN && x <= INT_MAX ? x : 0);
}
  ...
  for (;;)
    {
      ...
      switch (id)
        ...
        case 'i':
          {
            ...
            for (iter = orders; ; iter++)
              if (iter >= orders + INS_CNT)
                ...
                {
                  options->insert_order = (enum insert_order)(iter - orders);
                  break;
                }
          }
          break;
        ...
        case 'd':
          {
            ...
            for (iter = orders; ; iter++)
              if (iter >= orders + DEL_CNT)
                ...
                {
                  options->delete_order = (enum delete_order)(iter - orders);
                  break;
                }
          }
          break;
        ...
        case 'S':
          ...
          options->seed = (unsigned int)strtoul (arg, NULL, 0);
          break;
...
gen_insertions (size_t n, enum insert_order insert_order, int insert[])
{
  ...
  switch (insert_order)
    ...
    case INS_BALANCED:
      gen_balanced_tree (0, (int)(n - 1), &insert);
      break;
