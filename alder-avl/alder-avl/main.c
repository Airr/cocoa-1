/**
 * This file, main.c, is part of alder-avl.
 *
...
#include <stdlib.h>
#include "cmdline.h"
#include "alder_avl_option.h" 
#include "alder_avl_dictionary_interval.h"

int main(int argc, char * argv[])
{
    int isExitWithHelp = 0;
    alder_avl_option_t option;
    struct gengetopt_args_info args_info;
    if (my_cmdline_parser (argc, argv, &args_info) != 0) exit(1) ;
    isExitWithHelp = alder_avl_option_init(&option, &args_info);
    if (isExitWithHelp == 1) {
        my_cmdline_parser_free(&args_info);
        alder_avl_option_free(&option);
        exit(1);
    }
    
    alder_avl_dictionary_interval();
