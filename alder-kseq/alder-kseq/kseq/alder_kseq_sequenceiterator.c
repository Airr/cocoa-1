/**
 * This file, alder_kseq_sequenceiterator.c, is part of alder-kseq.
 *
...
enum alder_kseq_sequenceiterator_state {
    alder_kseq_sequenceiterator_state_begin,
    alder_kseq_sequenceiterator_state_open,
    alder_kseq_sequenceiterator_state_end
};

struct alder_kseq_sequenceiterator_struct {
    enum alder_kseq_sequenceiterator_state state;
...
static void
alder_kseq_sequenceiterator_init(alder_kseq_sequenceiterator_t *o);
...
alder_kseq_sequenceiterator_test(struct bstrList *fn)
{
    ...
    for (int i = 0; i < fn->qty; i++) {
        alder_kseq_sequenceiterator_open(fiter,
                                          bdata(fn->entry[i]));
        int token = alder_kseq_sequenceiterator_token(fiter);
        while (token <= ALDER_TOKEN_DNA_SIZE) {
            ...
            token = alder_kseq_sequenceiterator_token(fiter);
        }
...
alder_kseq_sequenceiterator_create()
{
    alder_kseq_sequenceiterator_t *o = malloc(sizeof(*o));
    ALDER_RETURN_NULL_IF_NULL(o);
    memset(o, 0, sizeof(*o));
    alder_kseq_sequenceiterator_init(o);
    return o;
}
...
{
    if (o != NULL) {
        alder_kseq_sequenceiterator_close(o);
        XFREE(o);
    }
}
