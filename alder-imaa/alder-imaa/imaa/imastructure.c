...
static void augment (void); 
static int hungarian (void);
...
static int cost[Nv][Nv];   /* cost matrix */
static int n, max_match;   /* n workers and n jobs */
static int lx[Nv], ly[Nv]; /* labels of X and Y parts */
static int xy[Nv];         /* xy[x] - vertex that is matched with x, */
static int yx[Nv];         /* yx[y] - vertex that is matched with y */
static int Sh[Nv], Th[Nv]; /* sets Sh and Th in algorithm */
static int slack[Nv];      /* as in the algorithm description */
static int slackx[Nv];     /* slackx[y] such a vertex, that */
                           /* l(slackx[y]) + l(y) - w(slackx[y],y) = slack[y] */
static int prev[Nv];       /* array for memorizing alternating paths */

void
alert (void)
{
  IM_errloc (AT, "memory allocation failure. try again");
}
