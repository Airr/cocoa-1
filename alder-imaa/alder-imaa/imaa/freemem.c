...
free_locus ()
{
  ...
  for (li = 0; li < nloci; li++)
  {
    ...
    {
      free_chainstate_record_updates_and_values (L[li].t_rec, 1);
    }
  }
}                               // free_locus
...
freeanymemory ()
{
  ...
  if (treeoptions[POPULATIONTREE] == 1)
  {
    free_value_record (gBDr->v);
    free_value_record (gBDx->v);
    free_chainstate_record_updates_and_values (gBDr, 1);
    free_chainstate_record_updates_and_values (gBDx, 1);
  }
  ...
  for (ci = 0; ci < numchains; ci++)
  {
    C[ci]->nnewickstring = 0;
    XFREE (C[ci]->newickstring);
