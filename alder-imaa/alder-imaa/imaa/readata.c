...
static int DNA2Index (int li, char c);
...
// is this trapping of eol really necessary ??? 
      ...
      while ((c = (char) fgetc (infile)) != '\n')
      {
        ...
        //if ((c != ' ') && (c != '\t'))
        {
          ...
          /* INA2Index increases pi[li][x] */
          L[li].seq[i][j] = DNA2Index (li, c);
          if (L[li].seq[i][j] == DNA_NUMBER)
          {
...
parse_locus_info (int li, int *uinext, char *cc, int *fpstri, char fpstr[])
{
  ...
  if (cc != NULL && cc[0] == 'A')
  {
    ...
    {
      IM_err (IMERR_INPUTFILEINVALID,
              "A# must be used with assignment feature");
    }
    L[li].numgenesunknown = atoi (&cc[1]);
    if (L[li].numgenesunknown < 0)
    {
      IM_err (IMERR_INPUTFILEINVALID,
              "Number in A# must be 0 or a positive ninteger: %s",
              cc);
    }
    cc = nextnonspace (cc);
  }
  ...
  if (assignmentoptions[POPULATIONASSIGNMENT] == 1)
  {
    ...
    total_numgenes += L[li].numgenesunknown;
  }
                              ...
                              char startpoptreestring[])
{
  ...
  if (assignmentoptions[POPULATIONASSIGNMENTINFINITE] == 1
      || npops == 1   /* why was this here? A: To set the two values. */)
  {
    lastperiodnumber = 1;
    numsplittimes = 0;
  }
          ...
          char fpstr[], int **numsitesIS)
{
  ...
  char textline[DATAFILEMAXLINELENGTH + 1];
  FILE *infile;
  int numloci; /* See the comment start_setup_L. */
  int useThisLocus;
  const char *sepSpace = " ";
  char *opt;
  int numgene;
  ...
  L = malloc (numloci * sizeof (struct locus));
  uinext = 0;
  SP "\nLocus Information\n");
  SP "-----------------\n");
  SP "\nNumber of loci: %d \n", numloci);
  ...
  li = -1;
  for (liInDataFile = 0; liInDataFile < nloci; liInDataFile++)
  {
    /* Need to use only loci in --part-loci. */ 
    if (imcmdoptions[IMCMD_PART_LOCI] == 1)
    {
      useThisLocus = 0;
      for (i = 0; i < gnumberPartloci; i++)
      {
        if (gpartloci[i] == liInDataFile + 1)
        {
          useThisLocus = 1;
          break;
        }
      ...
      {
        /* Skip this locus. */
        /* Find the number of genes and skip lines. */
        fgets (textline, DATAFILEMAXLINELENGTH, infile);
        i = 0; 
        numgene = 0;
        for (opt = strtok (textline, sepSpace); 
             opt; 
             opt = strtok (NULL, sepSpace))
        {
          if (1 <= i && i <= npops)
          {
             d = atoi (opt);
