...
#define RELEASE_DATE "October 13, 2010 with Assignment version Dececmber, 2011"
 ...
 * Usages: 
 *   -a1 : invoke population assignment
 *   -a2 : invoke population admixture
 *   -a3 : island model
 *   -a13: Assignment with island model
 *   -a23: Admixture with island model
 *   -a43 : DNA Barcoding with island model
 *   -a4: DNA barcoding only
 *   -a1 -j6 : Assignment with no-migration
 *   -a2 -j6 : Admixture with no-migration
 *   -a4 -j6 : DNA Barcoding with no-migration
 *   -a3 -j6 -> invalid option combinations
 *
...
enum
{
  ...
  POPULATIONASSIGNMENTADM            = 2,
  POPULATIONASSIGNMENTINFINITE       = 3,
  POPULATIONASSIGNMENTBAR            = 4,
...
enum
{
  ...
  IMCMD_HELP,                /* --help */
  IMCMD_PART_LOCI,           /* --part-loci 1-10 */
  IMCMD_INDIVIDUAL,          /* --individual data.individual */
  IMCMD_FINDMEANASSIGNMENT,  /* --findmeanassignment */
  IMCMD_NUMBER 
};
...
struct locus
{
  ...
  int pairs[MAXGENES];
  char gNames[MAXGENES][MAXLENGENENAME];       /* upto 1000 gene names of at most 9-character string */
  int numgenesknown;
  int numgenesunknown;
...
//added 3/10/10: r=lambda-mu ~ Uniform(0,1), and x=mu/lambda ~ 1/Uniform(0,100)
...
void makenewicktreestring (int ci, int curpop, char ** buildstr, 
                           size_t * lenbuildstr);
...
int swapchains (int swaptries, int heatmode, double hval1, double hval2);
