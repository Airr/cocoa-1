         ...
         int nomigration)
{
  ...
  *ptime += expo (start_coalesce_rate);
  ...
  /* SC: Wed Jun 15 01:07:01 EDT 2011
   * Do we have to have skip the while loop with island model?
   */
...
makeHKY (int ci, int li, int nosimmigration)
{
  ...
  for (i = L[li].numgenes; i < 2 * L[li].numgenes - 1; i++)
  {
    ...
    gtree[i].hkyi.frac = orig2d_alloc2Ddouble (L[li].numsites,4);
    gtree[i].hkyi.newfrac = orig2d_alloc2Ddouble (L[li].numsites,4);
  } 
