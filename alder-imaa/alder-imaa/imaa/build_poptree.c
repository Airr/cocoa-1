...
//static void poptreewrite (int ci, char *buildstr); We make it global.
...
/* We generate the Newick tree string of the population tree pointed by Chain
 * ci. */
/* FIXME: makepoptreestring is not global, which we call it from assignment.c.
 * Please, make it global. Make this function makenewicktreestring global as
 * well. */
/* We traverse the tree in postorder.
 */
void
makenewicktreestring (int ci, int curpop, char ** buildstr, size_t * lenbuildstr)
{
  int i;
  int ndaughter;
  int left;
  ...
  if (curpop == -1)
  {
    ...
    while (C[ci]->poptree[i].down != -1)
      i++;
    curpop = i; /* This is the root population label. */
  }

  ndaughter = ptree[curpop].numup;
  assert (ndaughter == 2 || ndaughter == 0);

  
  if (ndaughter == 2)
    {
      if (!(strlen (*buildstr) + 10 < *lenbuildstr))
        {
          *lenbuildstr += 10;
          *buildstr = realloc (*buildstr, *lenbuildstr * sizeof (char));
        }
      strcat (*buildstr, "(");

      left = ptree[curpop].up[0];
      makenewicktreestring (ci, left, buildstr, lenbuildstr);
      ...
      right = ptree[curpop].up[1];
      makenewicktreestring (ci, right, buildstr, lenbuildstr);
      if (!(strlen (*buildstr) + 10 < *lenbuildstr))
        {
          *lenbuildstr += 20;
          *buildstr = realloc (*buildstr, *lenbuildstr * sizeof (char));
      ...
      strcat (*buildstr, ")");
/*
      sprintf (numString, ")%d", curpop);
      strcat (*buildstr, numString);
*/
    }
  else
    {
      ...
      sprintf (numString, "%d", curpop + 1);
      strcat (*buildstr, numString);
    }

  if (ptree[curpop].down < 0)
    {
      if (!(strlen (*buildstr) + 10 < *lenbuildstr))
        {
          *lenbuildstr += 10;
    ...
    {
      left = ptree[curpop].up[0];
      if (!(strlen (*buildstr) + 30 < *lenbuildstr))
        {
          *lenbuildstr += 31;
          *buildstr = realloc (*buildstr, *lenbuildstr * sizeof (char));
        }
      if (left < 0)
        {
          sprintf (numString, ":%lf", ptree[curpop].time);
        }
      else
        {
          sprintf (numString, ":%lf", ptree[curpop].time - ptree[left].time);
        }
      strcat (*buildstr, numString);
    }
