...
init_after_start_IMA ()
{
  ...
  if (assignmentoptions[PRINTSTRUCTURAMA] == 1)
  {
    ...
    {
      imaStructuramaCreateInFile (Structurama, 0);
    }
    if (assignmentoptions[POPULATIONASSIGNMENTADM] == 1)
    {
      imaStructuramaCreateInFile (Structurama, 1);
    }
    imaStructuramaCreateBatFile (outfilename);
  }
  else if (treeoptions[POPULATIONTREE] == 1)
  {
    strcpy (Structurama, outfilename);
    strcat (Structurama, ".nex");
    IMA_convert_IM2Mrbayes (Structurama);
    IMA_output_mrbayes_bat (outfilename);
    sprintf (cmd, "%s %s.nex.parts", SYSCMDDELETE, outfilename);
    system (cmd); 
    sprintf (cmd, "%s %s.nex.con", SYSCMDDELETE, outfilename);
    system (cmd); 
    sprintf (cmd, "%s %s.nex.trprobs", SYSCMDDELETE, outfilename);
    system (cmd); 
  }
  ...
  {
    imaAdmixturePrintHeader (outfilename, 0, 0);
#ifdef DEBUG
    if (numchains > 1)
      {
        for (ci = 0; ci < numchains; ci++)
        {
          imaAdmixturePrintHeader (outfilename, ci, 1);
        }
      }
#endif /* DEBUG */
  }
  ...
  {
    recordptree_header (outfilename, 0);
  }
...
free_ima_main_stuff ()
{
  ...
  if (individualfilename != NULL)
  {
    XFREE (individualfilename);
  }
...
scan_commandline (int argc, char *argv[])
{
  ...
  char *opt;
  const char *sep = ",:";
  const char *dashSeperator = "-";
  int ioption;
  int numberComma;
  int numberCommaIterator;
  int numberDash;
  int numberDashIterator;
  int dashStartLocus;
  int dashEndLocus;
  ...
  {
    printf ("IMa2 Program - copyright 2011 by Jody Hey, Rasmus Nielsen and Sang Chul Choi\n");
    printf ("Version: %s\n", VERSION);
    ...
    printf ("    2  Invoke population admixture\n");
    printf ("    3  Island model\n");
/* Tue Jun 14 09:10:59 EDT 2011
   The relabel function for DNA barcoding uses getm and mwork functions that
   were changed in their prototypes. After fixing them we can use DNA barcoding
   feature.
    printf ("    4  Invoke DNA barcoding\n"); */
    ...
    //printf ("    7 Print histograms of the number of migration events and the mean time of migration events (MCMC mode only)\n");
    // 4/1/11  removed printing of distributions of splitting times
...
/*
    printf ("--beta [REAL NUMBER]\n");
    printf ("    beta value for a single heated chain\n");
    printf ("--gbeta [REAL NUMBER]\n");
    printf ("    global beta value for model comparison\n");
    printf ("--true-assignment\n");
    printf ("    true assignment that is compared with an estimate\n");
    printf ("--with-tree-inference\n");
    printf ("    let IMa2 infer population trees\n");
*/
    printf ("--three-point-turn\n");
    printf ("    use three-point-turn update; use with -a1 or -a2\n");
    printf ("--sample-phylogeny\n");
    printf ("    Split a posterior sample of species tree and assignment\n");
    printf ("--nloci [POSITIVE INTEGER]\n");
    printf ("    Uses only the first N loci\n");
    printf ("--part-loci [NUMBER-NUMBER||NUMBER,NUMBER,...]\n");
    printf ("    Uses some loci\n");
    printf ("--individual [FILENAME]\n");
    printf ("    Uses some loci\n");
    printf ("--findmeanassignment\n");
    printf ("    IMa command - find mean assignment\n");
    printf ("--help\n");
    printf ("    Print a complete help to screen\n");
...
*/
    ...
    for (i = 1; i < argc; i++)
    {
      ...
      if (ch == '-')
      {
        /* Long option is allowed. Options can have their option values, or some
         * of them do not have any. In the main header file of imamp.h add
         * IMCMD_OPTIONNAME for an option --optionname. If it does not need any
         * option value, then we set the imcmdoptions[IMCMD_OPTIONNAME] to 1 and
         * continue to process next options. See the option ---three-point-turn
         * for example. If an option requires a value, then set the 
         * imcmdoptions[IMCMD_OPTIONNAME] to 1 and go to case '-' of the switch
         * statement below to process the value. */
        if (strcmp(&pstr[2], "beta") == 0)
        {
          imcmdoptions[IMCMD_BETA] = 1;
          strcpy (pstr, argv[++i]);
          strcat (command_line, " ");
          strcat (command_line, pstr);
          beta[0] = atof (&pstr[0]);
        ...
        else if (strcmp(&pstr[2], "part-loci") == 0)
        {
          imcmdoptions[IMCMD_PART_LOCI] = 1;
          strcpy (pstr, argv[++i]);
          strcat (command_line, " ");
          strcat (command_line, pstr);
          numberComma = comma_exists (pstr);
          numberDash = imaUtilityDashExists (pstr);
          if (numberComma > 0 && numberDash > 0)
          {
            IM_errloc (AT, "option --part-loci is being misused.");
          }

          if (numberComma > 0)
          {
            gnumberPartloci = numberComma + 1;
            gpartloci = malloc (gnumberPartloci * sizeof (int));
            numberCommaIterator = 0;
            for (opt = strtok (pstr, sep); opt; opt = strtok (NULL, sep))
            {
              ioption = atoi (opt);
              gpartloci[numberCommaIterator] = ioption;
              numberCommaIterator++; 
            }
            assert (gnumberPartloci == numberCommaIterator); 
          }
          else if (numberDash == 1)
          {
            numberDashIterator = 0;
            for (opt = strtok (pstr, dashSeperator); opt; opt = strtok (NULL, dashSeperator))
            {
              if (numberDashIterator == 0)
              {
                dashStartLocus = atoi (opt);
              } 
              else if (numberDashIterator == 1)
              {
                dashEndLocus = atoi (opt);
              }
              numberDashIterator++; 
            }
            assert (numberDashIterator == 2);
            gnumberPartloci = dashEndLocus - dashStartLocus + 1;
            gpartloci = malloc (gnumberPartloci * sizeof (int));
            for (numberDashIterator = dashStartLocus;
                 numberDashIterator <= dashEndLocus; 
                 numberDashIterator++) 
            {
              gpartloci[numberDashIterator - dashStartLocus] = numberDashIterator;
            }
          }
          else
          {
            IM_errloc (AT, "option --part-loci 1-10 only one dash is allowed.");
          }
          /* nloci = gnumberPartloci; */
          continue;
        }
        else if (strcmp(&pstr[2], "individual") == 0)
        {
          imcmdoptions[IMCMD_INDIVIDUAL] = 1;
          strcpy (pstr, argv[++i]);
          strcat (command_line, " ");
          strcat (command_line, pstr);
          individualfilename = strdup (pstr);
          put_spaces_in_filepaths(individualfilename);
          continue;
        }
        else if (strcmp(&pstr[2], "gbeta") == 0)
        {
          imcmdoptions[IMCMD_GBETA] = 1;
          strcpy (pstr, argv[++i]);
          strcat (command_line, " ");
          strcat (command_line, pstr);
          gbeta = atof (&pstr[0]);
        ...
        else if (strcmp(&pstr[2], "true-assignment") == 0)
        {
          imcmdoptions[IMCMD_TRUE_ASSIGNMENT] = 1;
          strcpy (pstr, argv[++i]);
          strcat (command_line, " ");
          strcat (command_line, pstr);
        ...
        else if (strcmp(&pstr[2], "nloci") == 0)
        {
          imcmdoptions[IMCMD_NLOCI] = 1;
          strcpy (pstr, argv[i++]);
          strcpy (pstr, argv[i]);
          strcat (command_line, " ");
          strcat (command_line, pstr);
        ...
        else if (strcmp(&pstr[2], "with-tree-inference") == 0)
        {
          imcmdoptions[IMCMD_WITH_TREE_INFERENCE] = 1;
          continue;
        }
        else if (strcmp(&pstr[2], "sample-phylogeny") == 0)
        {
          imcmdoptions[IMCMD_SAMPLE_PHYLOGENY] = 1;
          continue;
        }
        else if (strcmp(&pstr[2], "three-point-turn") == 0)
        {
          imcmdoptions[IMCMD_TPT] = 1;
          continue;
        }
        else if (strcmp(&pstr[2], "help") == 0)
        {
          imcmdoptions[IMCMD_HELP] = 1;
          continue;
        }
        else if (strcmp(&pstr[2], "findmeanassignment") == 0)
        {
          imcmdoptions[IMCMD_FINDMEANASSIGNMENT] = 1;
          sIMaCommand = IMCMD_FINDMEANASSIGNMENT;
          continue;
        }
        else
        {
          IM_err (IMERR_COMMANDLINEFORMAT, "no such long option %s", pstr);
        }
      }
      ...
      if (strlen (argv[i]) == 2 
          || ch == '-'
          || (i < argc - 1 && isdigit (argv[i + 1][0])))  // space separates flag from its number
      {
        i++;
        strcpy (pstr, argv[i]);
        strcat (command_line, " ");
  ...
  if (imcmdoptions[IMCMD_FINDMEANASSIGNMENT] == 1)
  {
    if (!Ip)
    {
      IM_err (IMERR_MISSINGCOMMANDINFO, "No input file given on command line");
    }
    if (!Op)
    {
      strcpy (outfilename, infilename);
      strcat (outfilename, ".sum");
    }
    return;
  }

  if (imcmdoptions[IMCMD_HELP] == 1)
  {
    imaHelpPrint ();
    exit (0);
  }

  if (imcmdoptions[IMCMD_WITH_TREE_INFERENCE] == 1)
  {
    treeoptions[POPULATIONTREE] = 1;
  }
  else
  {
    treeoptions[POPULATIONTREE] = 0;
  }
 
  if (imcmdoptions[IMCMD_BETA] == 1 && (Hnp == 1 || numchains > 1))
  {
    IM_err (IMERR_COMMANDLINEFORMAT, "beta option is valid for a single chain");
  }
  ...
  if (Ap == 1)
  {
    ...
    if (assignmentoptions[POPULATIONASSIGNMENT] == 1
        ...
        || assignmentoptions[POPULATIONASSIGNMENTLOCAL] == 1
...
print_outputfile_info_string (void)
{
  ...
  *fpstri = 0;
  SP "IMa version 2.0 - Isolation with Migration Analysis  -  Jody Hey, Rasmus Nielsen, Sang Chul Choi 2011 \n");
  SP "Version: %s\n", VERSION);
  SP "Release date: %s\n\n",RELEASE_DATE);
  ...
  {
    // 4/1/11  removed printing of distributions of splitting times
    //SP "Distributions for migration event counts and times saved in file: %s%s\n",outfilename,".mpt");
...
void qupdate (void)
{
  ...
  /* update population assignment */
  if (imcmdoptions[IMCMD_TPT] == 1
      && (assignmentoptions[POPULATIONASSIGNMENT] == 1 || treeoptions[POPULATIONTREE] == 1)
      && step % 100 == 0
      ...
      && npops == 3)
  {
    for (ci = 0; ci < numchains; ci++)
    {
      updateassignmenttpt (ci);
    }
  }

  if (treeoptions[POPULATIONTREE] == 1)
  {
    for (ci = 0; ci < numchains; ci++)
    {
      updateassignmentptree (ci);
    }
  }
...
/* 12/17/09 spent a while playing with including the RY2 update.  It might help a little bit, but not much so don't bother.  */
      tupdatemethodpick = randposint (2);     // need to change this if DO_RY2UPDATE is defined (not much reason to do that though
      ...
      case 2:
        ...
        //changed = changet_RY3 (ci, periodpick);
#ifdef IMPROFILE
  ...
  /* BD Rates update for Div. Time. Prior */
  if (treeoptions[POPULATIONTREE] == 1)
  {
    for (ci = 0; ci < numchains; ci++)
    {
...
#endif
    qswapped =
      swapchains (swaptries, heatmode, hval1, hval2);
#ifdef IMPROFILE
...
void trendrecord (int loadarrayj)
{
  ...
  if (burndone && reset == 0)   // reset all trend-related values after burnin
  {
    ...
    if (treeoptions[POPULATIONTREE] == 1)
    {
      trend_reset (gBDr->v, 1);
      trend_reset (gBDx->v, 1);
    }
...
/* ADD ADDITONAL trend_reset() calls here */
  ...
  if (recordinc == recordtrendinc)
  {
    ...
    if (treeoptions[POPULATIONTREE] == 1)
    {
      inctrend (movespot, trendspot, gBDr->v, C[0]->bdr);
      inctrend (movespot, trendspot, gBDx->v, C[0]->bdx);
    }
...
void record (void)
{
  ...
  if (treeoptions[POPULATIONTREE] == 1)
  {
    recordval (gBDr->v, C[0]->bdr);
    recordval (gBDx->v, C[0]->bdx);
  }
...
void loadgenealogyvalues (void)
{
  ...
  nottoload = totalnumgenealogies - numtoload;
...
// STR ancestral allele states 
  ...
  {
    reclist[0] = L[0].t_rec;
    printacceptancerates (outto, 1, reclist, "Update Rates -- Tree");
  }

  if (treeoptions[POPULATIONTREE] == 1)
  {
    reclist[0] = gBDr;
    printacceptancerates (outto, 1, reclist, "Update Rates -- BD R");
    reclist[0] = gBDx;
    printacceptancerates (outto, 1, reclist, "Update Rates -- BD X");
  }
...
// assign
  ...
  {
    curvexy[j] = gBDr->v->xy;
    curvestr[j] = &gBDr->v->str[0];
    curve_do_logplot[j] = gBDr->v->do_logplot;
    nrecstep[j] = recordstep;
    j++;

    curvexy[j] = gBDx->v->xy;
    curvestr[j] = &gBDx->v->str[0];
    curve_do_logplot[j] = gBDx->v->do_logplot;
    nrecstep[j] = recordstep;
    j++;
  }
...
void callasciitrend (FILE * outtofile)
{
  ...
  if (treeoptions[POPULATIONTREE] == 1)
  {
    asciitrend (outtofile, gBDr->v, trenddoublepoint, trendspot);
    asciitrend (outtofile, gBDx->v, trenddoublepoint, trendspot);
  }
  return;
}                               // callasciitrend 
...
void check_to_record (void)
{
  ...
  if (j == savegenealogyint)
  {
    ...
    {
      imaAdmixturePrint (outfilename, 0);
    }
    else if (treeoptions[POPULATIONTREE] == 1)
    {
      recordptree (outfilename, 0);
    }
    genealogiessaved++;
    j = 1;
  }
...
{
  char filename1[FNSIZE];
  char filename2[FNSIZE];
  init_IMA ();
  scan_commandline (argc, argv);
  switch (sIMaCommand)
  {
    case IMCMD_FINDMEANASSIGNMENT:
      imaCmdFindmeanassignment (infilename, outfilename);
      imaCmdFindCoassignmentProbability (infilename, outfilename);
      return 0;
      break;
    default:
      break;
  ...
  {
    loadgenealogyvalues ();
    if (imcmdoptions[IMCMD_SAMPLE_PHYLOGENY] == 0)
    {
    ...
    else
    {
      ...
      {
        IM_errloc (AT, 
                   "Sampling phylogeny works with a population tree of three populations");
      }
      /* Based on assignment estimate split genealogy samples into three
       * phylogenies or three .ti files and .asn files. */
      imaPhylogenySplitTiAsn (outfilename);
    }
    return 0;
  }
  ...
  while (run ())
  {
    ...
    if (burndone)
    {
      ...
      /* FIXME PLEASE FIXME This average is simply wrong. I might have to try model
       * comparison again using Thermo. Dyn. Int. */
      gloglikelihood += C[0]->allpcalc.pdg; 
      gloglikelihood /= 2.0;
      glogprobg += C[0]->allpcalc.probg; 
      glogprobg /= 2.0;
    }
    step++;
//_CrtCheckMemory ();
      ...
      && assignmentoptions[POPULATIONASSIGNMENTASSIGNED] == 0)
  {
    ...
    {
      sprintf (filename1, "%s.asn", outfilename);
      sprintf (filename2, "%s.in.sum_assignments", outfilename);
      imaCmdFindmeanassignment (filename1, filename2);
      imaCmdFindCoassignmentProbability (filename1, filename2);
    }
    else
    {
      imaStructuramaLocalrun (outfilename);
      if (trueassignment != NULL)
        {
          imaStructuramaSummary (outfilename, 0);
        }
      imaStructuramaUncertainty (outfilename, 0); /* Argument for assignment estimate? */
    }
    IMAgenealogyAlign (outfilename, genealogiessaved);
  }
  printoutput ();
  ...
  if (treeoptions[POPULATIONTREE] == 1)
  {
    recordptree_footer (outfilename, 0);
  }
