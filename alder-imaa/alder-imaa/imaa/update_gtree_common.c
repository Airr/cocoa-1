...
void
hold_IMA_initmemory_edgemiginfo ()
{
...
*/
{
  ...
  int periodAtGtreeRootTime = findperiod (ci, G->roottime);
  if (assignmentoptions[POPULATIONASSIGNMENTINFINITE] == 0)
  {
    ncount += periodAtGtreeRootTime;
  }
  else if (periodAtGtreeRootTime != 0)
  {
    IM_errloc (AT, "Gene tree root time is too large!");
  }
  ...
  for (i = 0; i < 2 * ng - 1; i++)
  {
    ...
    while (gtree[i].mig[j].mt > -0.5)
    {
      ...
      assert (ISELEMENT (nowpop, C[0]->periodset[sgp->periodi])); // SCC: Why not ci but 0?
      sgp->pop = nowpop;
