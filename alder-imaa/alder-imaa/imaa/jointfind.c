...
/* fillplist is also defined in build_poptree.c         */
/* updateassignment.c uses fillplist in build_poptree.c */
/* The fillplist in build_poptree.c is now in imamp.h   */
/* This fillplist in jointfind has to be changed.       */
int jointfind_fillplist(char *c, int *plist, int startparamcount, int notused[]);
...
void setup_mapping(char *fname)// JH fixed signficiant bug in this functino 4/5/2010 
{
  ...
  while (fgets(modeltextline,MAXMODELTEXTLINE,nestedmodelfile)!= NULL) if (isalpha(modeltextline[0]))
  {
    ...
    if (strcmp(keyword,"equal")==0 || (strcmp(keyword,"constant")==0)) 
    {
      ...
      if (m_or_p == 'p')
        n = jointfind_fillplist(c,&plist[numm][0], 0, paramsnotused[inmodel]);
      if (m_or_p == 'm')
        n = jointfind_fillplist(c,&plist[numm][0],numpopsizeparams,paramsnotused[inmodel]);
      for (i=0;i<n;i++)
      {
        if (m_or_p == 'p')
...
double difeloop(int modelparams, int modelnum)
{
  ...
  do
  {
    if (gen > 0 &&  (int) (5*modelparams) * (gen/(int) (5*modelparams)) == gen) 
    {
      p = 1;
    }
    else 
...
void modelloop(int modelnum)
{
  int i,newstartloop,newstartloopcheck;
  int modelparams;
  int countloop;
  double local_pd;
  ...
  if (modelnum >= 0)
  {
    ...
    temp = jointp(popnest,calculate_ess, &effective_sample_size);
  }
  else
     temp = jointp(bestvals, calculate_ess,&effective_sample_size);
  calculate_ess = 0;
} // modelloop
  ...
  precalculates as much stuff as possible that will get reused later */ 
{
  ...
  int gin, iin, ii;
  double sum, p, q;
  static int ccp, fcp, hccp, mcp, fmp, qip, mip, probgp, plgp, tp, pdgp;
  static double numsplittimes_local, /* paramprior, Never used */ log_numtrees;
  static int init = 0;
  static int npop_and_m;
  ...
  {
    init = 1;
    npop_and_m = numpopsizeparams + nummigrateparams;
...
void findjointpeaks(FILE *outfile,char *outfilename, char *nestfname,int number_of_parameters)
{
  ...
  bestvals = malloc((nparams+1) * sizeof(double));
  setbounds();
  setuplist();
   ...
   // if (modeltypecounts[nowmodeltype] > 0)
    {
      ...
      switch (nowmodeltype)
      {
        case 0 : 
              ...
              break;
        case 1 : 
              holdparams = numpopsizeparams;
              nparamrange[0] = 0;
              nparamrange[1] = numpopsizeparams;
          break;
        case 2 : 
              holdparams = nummigrateparams;
      ...
      FP "%d\t%s\t%d\t-\t-",j,logpstrformat(-bestvals[nparams]), holdparams);
      FP "\t%s",logpstrformat(effective_sample_size));
    ...
    for (mi = 0;mi<num_nestedmodels;mi++) if (fullmodeltype[mi] == nowmodeltype)
    {
      ...
      FP "%d\t%s\t%d",j,logpstrformat(-bestvals[nparams]), nparams_nested[mi]);
      FP"\t%d",holdparams-nparams_nested[mi]);
      if (nestedmodel_boundary_in_model[mi])
        FP"*");
      FP "\t%s", logpstrformat(2*(bestvals[nparams]-holdml)));
      FP "\t%s",logpstrformat(effective_sample_size));
