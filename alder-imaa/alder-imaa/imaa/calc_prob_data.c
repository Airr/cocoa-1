...
static int IUPAC_DNA[DNA_NUMBER][5] = {
  {1, DNA_A, -1, -1, -1},
  {1, DNA_C, -1, -1, -1},
  {1, DNA_G, -1, -1, -1},
  {1, DNA_T, -1, -1, -1},
  ...
  {2, DNA_G, DNA_T, -1, -1},
  {2, DNA_A, DNA_C, -1, -1},
  {3, DNA_C, DNA_G, DNA_T, -1},
  {3, DNA_A, DNA_G, DNA_T, -1},
  {3, DNA_A, DNA_C, DNA_T, -1},
  {3, DNA_A, DNA_C, DNA_G, -1},
  {4, DNA_A, DNA_C, DNA_G, DNA_T}
};
...
static int makefrac_iupac (int ci, int li, int node, double mutrate, double kappa,
                     int e1, int e2, int e3, int e4);
...
double pijt(int ci, int li, double mutrate, double t, double kappa, int from, int to)
    ...
    if (from==to)
    ...
    else
      {
        s = 0;
        to_arg = to;
        for (k = 0; k < IUPAC_DNA[to_arg][0]; k++)
          {
            to = IUPAC_DNA[to_arg][k + 1];
    if  (to==0 || to==2) PIj=pi[0]+pi[2];
    else PIj=pi[1]+pi[3];
    /*A=kappa+1.0; */ /*F84*/
    A=1.0+PIj*(kappa-1.0); /*HKY85*/
    if (from==to)
            s += pi[to]+pi[to]*exp(-mutrate*t)*(1.0/PIj-1.0)+exp(-mutrate*t*A)*((PIj-pi[to])/PIj);
    else if (from+to==2 || from+to==4)
            s += pi[to]+pi[to]*(1.0/PIj-1.0)*exp(-mutrate*t)-(pi[to]/PIj)*exp(-mutrate*t*A);
    else s += pi[to]*(1.0-exp(-mutrate*t));
          }
        return s;
      }
    }
...
int makefrac_rasmus(int ci, int li, int node, double mutrate, double kappa, int e1, int e2, int e3, int e4)
    ...
    if (up1!=-1)
        {
        up2=gtree[node].up[1];
        i=makefrac_rasmus(ci,li,up1, mutrate, kappa, e1, e2, e3, e4);
        if (i==0) fracpoint1 = gtree[up1].hkyi.frac;
        else {fracpoint1 = gtree[up1].hkyi.newfrac; ret=1;}
        i=makefrac_rasmus(ci,li,up2, mutrate, kappa, e1, e2, e3, e4);
...
// Jim Long's unwrapped makefrac() - much faster than Rasmus's original makefrac() that used recursion 
// We allow ambiguous DNA code such as R for A or G.
int
makefrac_iupac (int ci, int li, int node, double mutrate, double kappa, int e1,
          int e2, int e3, int e4)
{
  int i, j, up1, up2, ret = 0;
  double **fracpoint1, **fracpoint2;
  ...
  {
    up2 = gtree[node].up[1];
    i = makefrac_iupac (ci, li, up1, mutrate, kappa, e1, e2, e3, e4);
    if (i == 0)
    {
      fracpoint1 = gtree[up1].hkyi.frac;
    }
    ...
    i = makefrac_iupac (ci, li, up2, mutrate, kappa, e1, e2, e3, e4);
    if (i == 0)
    {
      fracpoint2 = gtree[up2].hkyi.frac;
    }
...
/************************ MAKE SURE THIS IS CORRECT. *************************/
          ...
          {
            pijt1 = 0.0;
            to_arg = L[li].seq[up1][i];
            for (k = 0; k < IUPAC_DNA[to_arg][0]; k++)
            {  
              stmp[0] = IUPAC_DNA[to_arg][k + 1];
            ...
            pijt2 = 0.0;
            to_arg = L[li].seq[up2][i];
            for (k = 0; k < IUPAC_DNA[to_arg][0]; k++)
            {  
              stmp[1] = IUPAC_DNA[to_arg][k + 1];
...
/************************ MAKE SURE THIS IS CORRECT. *************************/
  ...
  return ret;
}                               // Jim Long's unwrapped makefrac() with IUPAC
               ...
               int e3, int e4)
{
  ...
  makefrac_iupac (ci, li, C[ci]->G[li].root, mutrate, kappa, e1, e2, e3, e4);
  for (i = 0; i < L[li].numsites; i++)
  {
    fracp = 0;
    for (j = 0; j < 4; j++)
