...
changer (int ci)
{
  ...
  metropolis = 0.0 - priorDivTime (ci, r, x);
  ...
  if (newr > m)
    ...
    newr = -newr;
  assert (0 < newr && newr < m);


  metropolis += priorDivTime (ci, newr, x);
  lweight = beta[ci] * metropolis;
  logU = log (uniform ());
...
changex (int ci)
{
  ...
  if (U > 0.5)
    newx = x + (2.0 * U - 1.0) * windowsize;
  else
    newx = x - windowsize * U * 2.0;

  if (newx > m)
    newx = 2.0 * m - newx;
  else if (newx < 0)
    newx = -newx;
  assert (0 < newx && newx < m);

  metropolis += priorDivTime (ci, r, newx);
  lweight = beta[ci] * metropolis;
  logU = log (uniform ());
  if (lweight > 0.0 || lweight > logU)
  {
