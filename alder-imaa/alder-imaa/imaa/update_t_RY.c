...
/* returns a changed time:
- if the time is < than the recent bound (i.t. upperbound t_u_prior)  
  nothing is changed
- else, 
  if time is less than oldt,  return a stretched/compressed time
  else (time is greater than oldt) return a time that is increased by diff
  */ 
double
...
changet_RY1 (int ci, int timeperiod)    // after Rannala and Yang (2003)  - rubberband method
{
  ...
  divtimeprior = -priorDivTime (ci, C[ci]->bdr, C[ci]->bdx);
  ...
  for (li = 0; li < nloci; li++)
  {
   ...
   //setedgeflag (ci, li, G, gtree, t_d, t_u, timeperiod, skipflag[li]); turn off 9/19/10
    for (i = 0; i < L[li].numlines; i++)
    {
      if (gtree[i].down != -1
         /* && skipflag[li][i] == 0 don't use with setedgeflag turn off 9/19/10*/ )
      {
  ...
  metropolishastingsterm = beta[ci] * (tpw + divtimeprior) + (ecd + emd) * log (t_d_hterm) +
  ...
  else
  {
    ...
    for (li = 0; li < nloci; li++)
    {
      ...
      {
        if (gtree[i].down != -1
            /*&& skipflag[li][i] == 0 use with setedgeflag turned off 9/19/10*/ )
        {
