...
extern int gnumberPartloci;
extern int *gpartloci;
...
static void init_t_rec (int li);
static void init_BD_rec ();
...
setup_iparams (void)
{
  ...
  if (modeloptions[PARAMETERSBYPERIOD])
  {
    for (i = 0, ii = 0, k = npops; i <= lastperiodnumber; i++, k--)
    {
      for (j = 0; j < k; j++)
      {
        assert (treeoptions[POPULATIONTREE] == 0);
        if (treeoptions[POPULATIONTREE] != 0)
          IM_errloc (AT, "Not implemented yet");
        pj = C[0]->plist[i][j];
    ...
    for (ii = 0; ii < numpopsizeparams; ii++)
    {
      ...
      if (assignmentoptions[POPULATIONASSIGNMENTINFINITE] == 1)
      {
        ...
        for (i = 0; i < lastperiodnumber; i++)
          for (j = 0; j < npops; j++)
          {
            if (ii == C[0]->plist[i][j]) /* Only one population is selected. */
            {
              itheta[ii].wp.p[ni] = i;
              itheta[ii].wp.r[ni] = j;
              ni++;
  ...
  {
    /* For each period k, a pair of populations i and j. */
...
init_t_rec (int li)
{
  int num_a_update_types = IM_UPDATE_TREE_TPT_NUMBER;

  L[li].t_rec = malloc (sizeof (struct chainstate_record_updates_and_values));
  sprintf (L[li].t_rec->str, "tree_%d", li);
  L[li].t_rec->num_uptypes = num_a_update_types;
  L[li].t_rec->upnames = malloc (num_a_update_types * sizeof (strnl));

  sprintf (L[li].t_rec->upnames[IM_UPDATE_TREE_TPT_TYPE1], "Three-Point-Turn");

  L[li].t_rec->upinf = calloc ((size_t) L[li].t_rec->num_uptypes, sizeof (struct update_rate_calc));
  L[li].t_rec->num_vals = 1;
  L[li].t_rec->v = malloc (L[li].t_rec->num_vals * sizeof (struct value_record));
  sprintf (L[li].t_rec->v->str, "t%2d_tree", li);
  strncpy (L[li].t_rec->v->strshort, L[li].t_rec->v->str, PARAMSTRLENSHORT - 1);
  L[li].t_rec->v->plotrange.min = 0;
  L[li].t_rec->v->plotrange.max = 10;
  L[li].t_rec->v->do_autoc = 0;
  L[li].t_rec->v->do_xyplot = 0;
  L[li].t_rec->v->do_trend = 0;
  L[li].t_rec->v->plotrescale = 1.0;
  L[li].t_rec->v->do_logplot = 0;
  init_value_record (L[li].t_rec->v, 0);
  return;
}                               /* init_t_rec */
...
init_BD_rec ()
{
  int num_a_update_types = 1;

  gBDr = malloc (sizeof (struct chainstate_record_updates_and_values));
  sprintf (gBDr->str, "Div. Time. R Update");
  gBDr->num_uptypes = num_a_update_types;
  gBDr->upnames = malloc (num_a_update_types * sizeof (strnl));
  sprintf (gBDr->upnames[0], "BD r");
  gBDr->win = 0.7;
  gBDr->pr.max = 1.0;
  gBDr->pr.min = 0.0;
  gBDr->upinf = calloc ((size_t) num_a_update_types, sizeof (struct update_rate_calc));
  gBDr->num_vals = num_a_update_types;
  gBDr->v = malloc (num_a_update_types * sizeof (struct value_record));
  sprintf (gBDr->v->str, "Div. Time. R");
  strncpy (gBDr->v->strshort, gBDr->v->str, PARAMSTRLENSHORT - 1);
  gBDr->v->plotrange.min = 0.0;
  gBDr->v->plotrange.max = 1.0;
  gBDr->v->do_autoc = 1;
  gBDr->v->do_xyplot = 1;
  gBDr->v->do_trend = 1;
  gBDr->v->plotrescale = 1.0;
  gBDr->v->do_logplot = 0;
  init_value_record (gBDr->v, 0);

  gBDx = malloc (sizeof (struct chainstate_record_updates_and_values));
  sprintf (gBDx->str, "Div. Time. X Update");
  gBDx->num_uptypes = num_a_update_types;
  gBDx->upnames = malloc (num_a_update_types * sizeof (strnl));
  sprintf (gBDx->upnames[0], "BD x");
  gBDx->win = 10.0;
  gBDx->pr.max = 100.0;
  gBDx->pr.min = 0.0;
  gBDx->upinf = calloc ((size_t) num_a_update_types, sizeof (struct update_rate_calc));
  gBDx->num_vals = num_a_update_types;
  gBDx->v = malloc (num_a_update_types * sizeof (struct value_record));
  sprintf (gBDx->v->str, "Div. Time. X");
  strncpy (gBDx->v->strshort, gBDx->v->str, PARAMSTRLENSHORT - 1);
  gBDx->v->plotrange.min = 0.0;
  gBDx->v->plotrange.max = 100.0;
  gBDx->v->do_autoc = 1;
  gBDx->v->do_xyplot = 1;
  gBDx->v->do_trend = 1;
  gBDx->v->plotrescale = 1.0;
  gBDx->v->do_logplot = 0;
  init_value_record (gBDx->v, 0);
}
...
{
  int li, i;
  /* SC: Wed Jun 29 10:18:23 EDT 2011
   * I want use parts of loci not all of them.
   * The option --part-loci allows users to choose some of the loci. I need to
   * deal with nloci variable. The acutal total number of loci is still nloci.
   * I still need the total number of loci in the input file. In the processing
   * of an input file nloci might mean differently in different places. */
  int numloci;
  ...
  if (imcmdoptions[IMCMD_PART_LOCI] == 0)
  {
    numloci = nloci;
  }
  else
  {
    numloci = gnumberPartloci;
  }
  numsitesIS = malloc (numloci * sizeof (int *));
  uvals = malloc (numloci * sizeof (double *));   // rows in this matrix are malloced in setup_L
  for (i = 0; i < nloci; i++)
    numsitesIS[i] = calloc ((size_t) npops + 1, sizeof (int));
  readdata (infilename, fpstri, fpstr, numsitesIS);
  /* SC: Wed Jun 29 11:07:25 EDT 2011
   * Are we done with reading a data file?
   */
  for (li = 0; li < nloci; li++)
  {
...
start_setup_C (void)
{
  ...
  for (ci = 0; ci < numchains; ci++)
  {
    ...
    {
      C[ci]->nasn = malloc (npops * nloci * sizeof (int));
    }
    C[ci]->popndown = NULL;
    C[ci]->popnmig = NULL;
    C[ci]->bdr = 0.2;
  ...
  for (ci = 0; ci < numchains; ci++)
  {
    ...
    {
      assert (treeoptions[POPULATIONTREE] == 1);
      setup_poptree (ci, startpoptreestring);
      // sim_poptree (ci); FIXME: we need to generate a random initial tree.
      Phylogeny_setPopndown (&C[ci]->popndown, C[ci]);
      Phylogeny_setPopnmig (&C[ci]->popnmig, C[ci]);
    }
    C[ci]->nnewickstring = 50;
    C[ci]->newickstring = malloc (C[ci]->nnewickstring * sizeof (char));
...
finish_setup_C ()
{
  ...
  /* Tree or Assignment, or neither. */
  assert ((treeoptions[POPULATIONTREE] 
           + assignmentoptions[POPULATIONASSIGNMENT] == 1) 
          || (treeoptions[POPULATIONTREE] 
              + assignmentoptions[POPULATIONASSIGNMENT] == 0));
  if (treeoptions[POPULATIONTREE] == 1)
  {
    /* Population tree is set here. */
    imaTreeSet ();
  }
  if (assignmentoptions[POPULATIONASSIGNMENT] == 1)
  {
...
setup (char infilename[], int *fpstri, char fpstr[], char priorfilename[])
{
      ...
      || npops == 1)
  {
    T = NULL;
    tprior = 0.0;
    setup_T (); /* SC: Do we have to call setup_T? */
  }
  else
  {
    setup_T ();
