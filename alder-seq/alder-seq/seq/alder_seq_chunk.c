/**
 * This file, alder_seq_chunk.c, is part of alder-seq.
 *
...
alder_seq_chunk_test(char **inputs, unsigned int inputs_num)
{
    ...
    for (unsigned int i = 0; i < inputs_num; i++) {
        ...
        lenBuf2 = 0;
        s = alder_seq_chunk(&lenBuf, buf, sizeBuf,
                ...
                size_t sizeTailbuf, FILE *fp)
{
    ...
    size_t atEndline = 0;
    for (atEndline = 0;
         atEndline < sizeBuf2 && atEndline < sizeBuf;
         atEndline++) {
        if (buf[*lenBuf2 + lenRead - 1 - atEndline] == '\n') {
            foundEndline = true;
            break;
        }
    }
    if (atEndline == *lenBuf2 + lenRead - 1) {
        alder_log5("First is '<': angle turned out to be not found!");
        foundEndline = false;
    }
    // 3
    if (foundEndline == true) {
        // Copy a tail of buf to buf2.
        *lenBuf = *lenBuf2 + lenRead - (atEndline + 1);
        *lenBuf2 = atEndline + 0;
        if (*lenBuf2 > 0) {
            memcpy(buf2, buf + *lenBuf + 1, *lenBuf2);
        }
