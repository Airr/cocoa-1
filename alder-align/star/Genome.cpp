...
//         system("echo `date` ..... Finished genome loading >> Log.timing.out");
    ...
    if (P->alignIntronMax==0 && P->alignMatesGapMax==0) {
        ...
        P->winBinNbits=max( (uint) ceil(log2(P->nGenome/40000)-0.5), (uint) ceil(log2(max(P->alignIntronMax,P->alignMatesGapMax)/4)-0.5) );
    ...
    if (P->alignIntronMax==0) P->alignIntronMax=1000000000;
    if (P->alignMatesGapMax==0) P->alignMatesGapMax=1000000000;    
