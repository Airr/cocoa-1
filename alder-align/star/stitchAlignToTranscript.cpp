...
//                             trA->rMM[trA->nMM + nMM] = rAend+ii;                
    ...
    //                         if (Q[rAend+ii]>=P->Qgood) nMMgood++;                        
                ...
                if (Del>P->alignIntronMax) return -1000003; //large gaps not allowed
                ...
                do { // 2. scan to the right to find the best junction locus
                    ...
                    if (Del>=P->alignIntronMin || P->sjdbN>0) {//only check intron motif for large gaps= non-Dels
                ...
                if (jCan<=0) {//flush deleteinos and non-canonical junction to the left
                    jR-=jjL;
...
//             if ( Score>0 && nMM<=200 )
            ...
            if (gBstart > trA->exons[trA->nExons-1][EX_G] + trA->exons[trA->nExons-1][EX_L] + P->alignMatesGapMax) return -100004; //gap between mates too large
            ...
            trExtend.reset();
            if ( extendAlign(R, Q, G, rAend+1, gAend+1, 1, 1, MAX_READ_LENGTH, trA->nMatch, trA->nMM, P->outFilterMismatchNmax, P->outFilterMismatchNoverLmax, &trExtend) ) {
