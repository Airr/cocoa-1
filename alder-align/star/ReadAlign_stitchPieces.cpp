...
//     };
    ...
    if (P->chimSegmentMin>0 && nW>1 && trBest->rLength >= P->chimSegmentMin \
        ...
        for (uint iW=0;iW<nW;iW++) {//check all other windows for chimeras
            ...
            if (trBest==trAll[iW][0] || trAll[iW][0]->intronMotifs[0]>0) continue; //do not stitch a window to itself, or to a window with non-canonical junctions
            uint chimStr1;
            if (trAll[iW][0]->intronMotifs[1]==0 && trAll[iW][0]->intronMotifs[2]==0) {//strand is undefined
                chimStr1=0;
            } else if ( (trAll[iW][0]->Str==0) == (trAll[iW][0]->intronMotifs[1]>0)) {//strand the same as RNA
                chimStr1=1;
            } else {//strand opposite to RNA
                chimStr1=2;
            };            
            
            if (chimStr!=0 && chimStr1!=0 && chimStr!=chimStr1) continue; //chimeric segments have to have sonsitent strands
            
            uint roStart2=trAll[iW][0]->Str==0 ? trAll[iW][0]->exons[0][EX_R] : Lread - trAll[iW][0]->exons[trAll[iW][0]->nExons-1][EX_R] - trAll[iW][0]->exons[trAll[iW][0]->nExons-1][EX_L];
            uint roEnd2=trAll[iW][0]->Str==0 ? trAll[iW][0]->exons[trAll[iW][0]->nExons-1][EX_R] + trAll[iW][0]->exons[trAll[iW][0]->nExons-1][EX_L] - 1 : Lread - trAll[iW][0]->exons[0][EX_R] - 1;
            ...
            if (roEnd1 > P->chimSegmentMin + roStart1 + chimOverlap && roEnd2> P->chimSegmentMin + roStart2 + chimOverlap  \
                ...
                int chimScore=trBest->maxScore + trAll[iW][0]->maxScore - (int)chimOverlap; //subtract overlap to avoid double counting
        ...
        if (chimScoreNext + P->chimScoreSeparation < chimScoreBest) {//report only if chimera is unique
            ...
            if ( chimN==2 && trChim[0].exons[e0][EX_L]>=P->chimJunctionOverhangMin+chimRepeat0 && trChim[1].exons[e1][EX_L]>=P->chimJunctionOverhangMin+chimRepeat1 ) {//standard SAM output single end reads, unique chimeras only
                ...
                chunkOutChimJunction << P->chrName[trChim[0].Chr] <<"\t"<< chimJ0 - P->chrStart[trChim[0].Chr]+1 <<"\t"<< (trChim[0].Str==0 ? "+":"-") \
                        ...
                        <<"\t"<< trChim[1].exons[0][EX_G] - P->chrStart[trChim[1].Chr]+1 <<"\t"<<  outputTranscriptCIGARp(trChim[1]) <<endl;
            };
        };//chimeric score
    };//chimeric search
};//end of function
