...
void stitchWindowAligns(uint iA, uint nA, int Score, bool WAincl[], uint tR2, uint tG2, Transcript trA, \
    ...
    if (iA>=nA) {//no more aligns to add, finalize the transcript
        ...
        if (trA.sjAnnot[trA.nExons-2]==1 && trA.exons[trA.nExons-1][EX_L] < P->alignSJDBoverhangMin) return; //this exon was not checkedin the cyle above
        ...
        if ( trA.exons[0][EX_iFrag]!=trA.exons[trA.nExons-1][EX_iFrag] ) {//check for correct overlap between mates
            ...
            if (trA.exons[iexM2][EX_G] < trA.exons[iexM2-1][EX_G]) {//mates overlap - check consistency of junctions
                uint iex2=iexM2;
                for (uint iex1=0;iex1<iexM2-1;iex1++) {//find the first exon of the second mate
                    if (trA.exons[iex1+1][EX_G] < trA.exons[iexM2][EX_G]) continue; // this exon is upstream of the second mate
                    if (trA.exons[iex1][EX_G] >= trA.exons[trA.nExons-1][EX_G]+trA.exons[trA.nExons-1][EX_L]-1) break; //this exon is downstream of the second mate
                    if (trA.canonSJ[iex1]>=0) {//this is a junction, compare with the first junction of the second mate
                       for (; iex2<trA.nExons-1; iex2++) {//cycle over mate2 exons
                           if (trA.canonSJ[iex2]>=0) {//mate2 junction
                               if ( trA.exons[iex1][EX_G]+trA.exons[iex1][EX_L] == trA.exons[iex2][EX_G]+trA.exons[iex2][EX_L] \
                                 && trA.exons[iex1+1][EX_G]==trA.exons[iex2+1][EX_G]) break;
                           };
                       };
                       if (iex2==trA.nExons-1) {
                           return; //did not find junction in mate1 to match mate2
                       };
                    };
                };
            };
        ...
        if (Score+P->outFilterMultimapScoreRange >= wTr[0]->maxScore || ( trA.iFrag>=0 && Score+P->outFilterMultimapScoreRange >= RA->maxScoreMate[trA.iFrag] )) {//only record the transcripts within the window that are in the Score range
...
//                     if (Score>wTr[iTr]->maxScore) break;
