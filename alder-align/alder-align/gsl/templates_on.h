...
#elif defined(BASE_ALDER_JUNCTION)
#define BASE alder_junction
#define SHORT junction
#define ATOMIC alder_junction
#define MULTIPLICITY 1
#define IN_FORMAT "%s"
#define OUT_FORMAT "%s"
#define ATOMIC_IO ATOMIC
#define ZERO (alder_junction){ .start = 0LLU, .gap   = 0LU, .strand= 0, .motif = 0, .annot = 0, .countUnique = 0LU, .countMultiple = 0LLU, .overhangLeft = 0U, .overhangRight = 0U }
#define ONE 1UL
#define UNSIGNED 1
