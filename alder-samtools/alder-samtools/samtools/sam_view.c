...
int main_samview(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "SbBct:h1Ho:q:f:F:ul:r:xX?T:R:L:s:Q:@:m:")) >= 0) {
		switch (c) {
		case 's':
			if ((g_subsam_seed = (uint32_t)strtol(optarg, &q, 10)) != 0) {
				srand(g_subsam_seed);
				g_subsam_seed = rand();
			}
		...
		case 'f': g_flag_on = (int)strtol(optarg, 0, 0); break;
		case 'F': g_flag_off = (int)strtol(optarg, 0, 0); break;
		...
		case '@': n_threads = (int)strtol(optarg, 0, 0); break;
