/**
 * This file, alder_palign_dialign.c, is part of alder-palign.
 *
 * Original Authors of DIALIGN-TX: greedy and progressive approaches for 
...
struct alignment* guided_aligner_rec(struct alignment *palgn, struct seq_col *scol, struct diag_col *dcol, struct scr_matrix* smatrix, struct prob_dist *pdist, struct gt_node *gtn, double thres_weight, char doHigher, int round)
{
    ...
    if(n1->isLeaf && n2->isLeaf) {
        ...
        //        alder_palign_dialign_print_seq_col(scol);
        //        alder_palign_dialign_print_diag_col(&tdcol);
        //        alder_palign_dialign_print_scr_matrix(smatrix);
        //        alder_palign_dialign_print_prob_dist(pdist);
        //        alder_palign_dialign_print_alignment(palgn);
...
void alder_palign_dialign_free_scr_matrix(struct scr_matrix *o)
{
    //struct scr_matrix
    //{
...
void alder_palign_dialign_free_prob_dist(struct prob_dist *o)
{
    //struct prob_dist
    //{
...
void alder_palign_dialign_free_seq(struct seq *o)
{
    //struct seq
    //{
...
void alder_palign_dialign_free_seq_col(struct seq_col *o)
{
    //struct seq_col
    //{
    ...
    for (int i = 0; i < o->length; i++) {
        free(o->seqs[i].data);
        free(o->seqs[i].name);
        //        alder_palign_dialign_free_seq(o->seqs)
    }
    free(o->seqs);
    free(o);
}
...
int alder_palign_dialign_initialize_from_file()
{
    init_parameters();
    set_parameters_dna();
...
int alder_palign_dialign_initialize()
{
    init_parameters();
    set_parameters_dna();
...
int alder_palign_dialign_finalize()
{
    alder_palign_dialign_free_prob_dist(pdist); pdist = NULL;
    alder_palign_dialign_free_scr_matrix(smatrix); smatrix = NULL;
    free_parameters();
    return 0;
}

int alder_palign_dialign_execute_with_anchor(alder_sam_t *sam,
                                             ...
                                             const char *queryName)
{
    ...
    const char *seqReference = bdata(palign->data[palignIndex].subject);
    ...
    for (size_t j = 0; j < numberOfAnchor; j++) {
        anchor1[j] = (int)(palign->data[palignIndex].qanchor->data[j].s);
        anchor2[j] = (int)(palign->data[palignIndex].anchor->data[j].s - palign->data[palignIndex].s);
        anchorLength[j] = (int)(palign->data[palignIndex].qanchor->data[j].e - palign->data[palignIndex].qanchor->data[j].s);
        assert(palign->data[palignIndex].anchor->data[j].e - palign->data[palignIndex].anchor->data[j].s == anchorLength[j]);
        anchorScore[j] = 10.0;
    }
    
    // This used to be the start of the function.
     ...
     */
    alder_palign_dialign_convert_to_sam(sam, fS, startReference, algn);
    
    
    free_alignment(algn);
    alder_palign_dialign_free_seq_col(in_seq_col); in_seq_col = NULL;
...
int alder_palign_dialign_execute(const char *seqReference, const char *seqRead)
{
    struct seq_col *in_seq_col = read_fasta2(seqReference, seqRead);
 ...
 */
void alder_palign_dialign_convert_to_sam(alder_sam_t *sam,
                                         const alder_fasta_t *fS,
                                         int64_t startReference,
