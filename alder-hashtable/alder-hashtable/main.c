/**
 * This file, main.c, is part of alder-hashtable.
 *
...
#include "alder_hashtable_option.h"

int main(int argc, char * argv[])
{
    int isExitWithHelp = 0;
    alder_hashtable_option_t option;
    struct gengetopt_args_info args_info;
    if (my_cmdline_parser (argc, argv, &args_info) != 0) exit(1) ;
    isExitWithHelp = alder_hashtable_option_init(&option, &args_info);
    if (isExitWithHelp == 1) {
        my_cmdline_parser_free(&args_info);
        alder_hashtable_option_free(&option);
        exit(1);
    }
    
    if (!(strcmp(args_info.inputs[0], "first"))) {
        alder_hashtable00();
    } else if (!(strcmp(args_info.inputs[0], "second"))) {
        alder_hashtable01();
    } else {
        printf("No command?\n");
    }
